...
public class VideoFragment extends Fragment implements TracksFragment.ITrackHolder {
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    getMenuInflater().inflate(tv.danmaku.ijk.media.example.R.menu.menu_player, menu);
    return true;
  }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    if (id == tv.danmaku.ijk.media.example.R.id.action_toggle_ratio) {
      int aspectRatio = mVideoView.toggleAspectRatio();
      String aspectRatioText = MeasureHelper.getAspectRatioText(getContext(), aspectRatio);
      mToastTextView.setText(aspectRatioText);
      mMediaController.showOnce(mToastTextView);
      return true;
    } else if (id == tv.danmaku.ijk.media.example.R.id.action_toggle_player) {
      int player = mVideoView.togglePlayer();
      String playerText = IjkVideoView.getPlayerText(getContext(), player);
      mToastTextView.setText(playerText);
      mMediaController.showOnce(mToastTextView);
      return true;
    } else if (id == tv.danmaku.ijk.media.example.R.id.action_toggle_render) {
      int render = mVideoView.toggleRender();
      String renderText = IjkVideoView.getRenderText(getContext(), render);
      mToastTextView.setText(renderText);
      mMediaController.showOnce(mToastTextView);
      return true;
    } else if (id == tv.danmaku.ijk.media.example.R.id.action_show_info) {
      mVideoView.showMediaInfo();
    } else if (id == tv.danmaku.ijk.media.example.R.id.action_show_tracks) {
  if (mDrawerLayout.isDrawerOpen(mRightDrawer)) {
    Fragment
        f =
        getFragmentManager().findFragmentById(tv.danmaku.ijk.media.example.R.id.right_drawer);
    if (f != null) {
    ...
    FragmentTransaction transaction = getFragmentManager().beginTransaction();
    transaction.replace(tv.danmaku.ijk.media.example.R.id.right_drawer, f);
    transaction.commit();
