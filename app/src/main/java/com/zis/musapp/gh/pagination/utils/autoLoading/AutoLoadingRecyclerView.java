 ...
 * the License.
 */
package com.zis.musapp.gh.pagination.utils.autoLoading;
...
public class AutoLoadingRecyclerView<T> extends RecyclerView {
  ...
  protected int getLastVisibleItemPosition() {
    ...
    throw new AutoLoadingRecyclerViewExceptions(
        "Unknown LayoutManager class: " + recyclerViewLMClass.toString());
  }

  public int getLimit() {
    if (limit <= 0) {
      throw new AutoLoadingRecyclerViewExceptions(
          "limit must be initialised! And limit must be more than zero!");
    }
    return limit;
  }
  ...
  public ILoading<T> getLoadingObservable() {
    if (iLoading == null) {
      throw new AutoLoadingRecyclerViewExceptions(
          "Null LoadingObservable. Please initialise LoadingObservable!");
    }
    return iLoading;
  }
  ...
  protected void onDetachedFromWindow() {
    ...
    if (subscribeToLoadingChannelSubscription != null
        && !subscribeToLoadingChannelSubscription.isUnsubscribed()) {
